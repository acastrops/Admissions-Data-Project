...
admit_raw <- fread("admitted.csv", sep = ",")
str(admit_raw)
...
```{r}
denied_raw <- fread("denied.csv", sep = ",")
str(denied_raw)
...
GPA_binned <- cut(applied_clean$GPA, breaks = seq(0,5,by = 0.5), labels = GPA_bin_labels, right = FALSE, include.lowest = TRUE)
...
SAT_binned <- cut(applied_clean$SAT, breaks = seq(400,1600,by = 100), labels = SAT_bin_labels, right = FALSE, include.lowest = TRUE)
...
ACT_binned <- cut(applied_clean$ACT, breaks = seq(0,36,by = 3), labels = ACT_bin_labels, right = FALSE, include.lowest = TRUE)
